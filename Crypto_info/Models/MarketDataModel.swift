...
//		if let item = marketCapPercentage.first(where: {$0.key == "btc"}) {
		...
		//		}
		//		return ""
		getValue(items: marketCapPercentage, key: "btc")
	}
	
	private func getValue(items: Dictionary<String, Double>, key: String? = "usd") -> String {
//	private func getValue(items: [String : Double]) -> String {
		if let item = items.first(where: {$0.key == key}) {
			return key == "usd" ? "\(item.value.formattedWithAbbreviations())" : "\(item.value.asPercentString())"
		}
		return ""
	}
}
